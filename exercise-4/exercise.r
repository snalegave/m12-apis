...
# are available.
query.params <- list(t = movie, r = "json")
...
movie.data.df <- as.data.frame(movie.data)
...
Director <- function(movie.info){
  return(paste(movie.info$Title, "was directed by", movie.info$Director))
}

# Call Director with your favorite movie, and assign it to the variable movie.director

movie.director <- Director(movie.data.df)
